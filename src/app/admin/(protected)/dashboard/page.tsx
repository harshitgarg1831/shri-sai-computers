...
export default async function AdminDashboard() {
    ...
    return (
        <div>
            <h1 className="text-3xl font-extrabold text-slate-900 mb-8">Dashboard Overview</h1>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex items-center justify-between">
                    <div>
                        <p className="text-slate-500 font-medium mb-1">Total Products</p>
                        <h3 className="text-3xl font-black text-slate-900">{productCount}</h3>
                    </div>
                    <div className="w-12 h-12 bg-blue-50 text-blue-600 rounded-xl flex items-center justify-center text-xl">📦</div>
                </div>
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex items-center justify-between">
                    <div>
                        <p className="text-slate-500 font-medium mb-1">Inquiries</p>
                        <h3 className="text-3xl font-black text-slate-900">{inquiryCount}</h3>
                    </div>
                    <div className="w-12 h-12 bg-green-50 text-green-600 rounded-xl flex items-center justify-center text-xl">💬</div>
                </div>
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex items-center justify-between">
                    <div>
                        <p className="text-slate-500 font-medium mb-1">Repair Requests</p>
                        <h3 className="text-3xl font-black text-slate-900">{repairCount}</h3>
                    </div>
            ...
            <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100">
                ...
                <div className="space-y-4">
                    {topRecents.length === 0 && <p className="text-slate-500">No recent activity found.</p>}
